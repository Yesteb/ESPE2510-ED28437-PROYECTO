...
namespace {
    ...
    void handleClient(int clientSock) {
        ...
        while (true) {
            ...
            std::cout << "[Server] Cliente sock=" << clientSock << " -> Peticion recibida: '" << req << "'\n";
            ...
            if (req.rfind("RESERVE|", 0) == 0) {
                ...
                std::lock_guard<std::mutex> lock(mtx);
                std::cout << "[Server] Procesando RESERVE cedula=" << cedula << " codigo=" << codigo << " nombre='" << nombre << "' tipo=" << tipo << "\n";
                // primero validar cedula ya no exista
                if (hashGlobal.existecedula(cedula)) {
                    std::string resp = "ERR|CEDULA_EXISTE\n";
                    std::cout << "[Server] RESPUESTA -> CEDULA_EXISTE para " << cedula << "\n";
                    send(clientSock, resp.c_str(), resp.size(), 0);
                    continue;
                }
                ...
                if (asientoOcupado(codigo)) {
                    ...
                    std::cout << "[Server] RESPUESTA -> ASIENTO_OCUPADO para codigo=" << codigo << "\n";
                    send(clientSock, resp.c_str(), resp.size(), 0);
                    continue;
                }
                ...
                std::cout << "[Server] RESPUESTA -> OK, reserva guardada codigo=" << codigo << " cedula=" << cedula << "\n";
                send(clientSock, resp.c_str(), resp.size(), 0);
                continue;
            }
