...
bool Validacion::validarNombre(const string& nombre) {
  ...
  //nombre con spacios
    ...
    // espacios con letras y sin numeros
    for (char c : nombre) {
        if (!isalpha(c) && c != ' ')
            return false;
    }
...
void Usuario::ingresarDatos() {
    ...
    do {
        ...
        if (!valido)
            cout << "Error: la cedula no es valida. Debe tener 10 digitos.\n";
    } while (!valido);
    cedula = auxiliar;
    ...
    while (true) {
        email = validarEmail.ingresar_email("Ingrese correo electronico: ");
    ...
    cout << "\nCorreo invalido. Intente nuevamente.\n";
}
}
/* 
    do {
        ...
        if (!valido)
            cout << "Error: el correo no es valido. Debe tener formato correcto (ej: ejemplo@correo.com).\n";
    } while (!valido);
    correo = auxiliar;
